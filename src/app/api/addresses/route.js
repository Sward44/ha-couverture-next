...
export async function POST(request) {
  ...
  try {
    ...
    if(!address) {
      console.log("Aucune adresse trouvée pour cet utilisateur. Création d'une nouvelle adresse.");
      address = new AddressModel({
        userId: chiffrage.userId,
        devisId: chiffrage.devisId,
        address: body.voie,
      ...
      console.log("Adresse trouvée dans MongoDB : ", address);
      const isAddressDifferent = address.address !== body.voie || address.code_postal !== body.codePostal || address.ville !== body.ville;

      if (isAddressDifferent) {
        console.log("Adresse différente, création d'une nouvelle entrée d'adresse.");
        address = new AddressModel({
          userId: chiffrage.userId,
          devisId: chiffrage.devisId,
          address: body.voie,
    ...
    await email.getTemplate("email-devis", {
      ...
      metadata: {
        ...
        ownerImages: image.map((item) => ({
          name: item.name,
          path: `${process.env.NEXT_PUBLIC_HOST}/uploads/${item.pictureId}.${item.type.slice(item.type.lastIndexOf('/') + 1)}`
        })),
        siteUrl: `${process.env.NEXT_PUBLIC_HOST}`,
      },
    });
