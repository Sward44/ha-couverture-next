    ...
    async jwt({ token, user, account, profile }) {
      if (user) {
        if (account?.provider === "google") {
          ...
          if (!userPerso) {
            let role = "user";
            if (profile.email === "davidlaunay567@gmail.com" || profile.email === "ha.couverture44@gmail.com") {
              role = "admin";
            }
            ...
            await UserModel.findOneAndUpdate(
              ...
              {
                $set: {
                  firstName: userPerso?.firstName ? userPerso.firstName : profile.given_name,
                  lastName: userPerso?.lastName ? userPerso.lastName : profile.family_name,
                  image: profile.picture,
                  emailVerified: userPerso.emailVerified === null && profile.email_verified === true ? new Date() : userPerso.emailVerified,
                },
              },
              { upsert: true }
            ).lean().exec();
